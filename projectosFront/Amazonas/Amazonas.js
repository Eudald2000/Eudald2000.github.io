  ...
  for (let f = 0; f < board.children.length; f++) {
    for (let c = 0; c < board.children[f].children.length; c++) {
      if (turnoActual % 2 == 0 && board.children[f].children[c].children[0]) {
        if (
          board.children[f].children[c].children[0].getAttribute("src") ==
          "img/blanca.png"
        ) {
        ...
        turnoActual % 2 != 0 &&
        board.children[f].children[c].children[0]
      ) {
        if (
          board.children[f].children[c].children[0].getAttribute("src") ==
          "img/negra.png"
...
function pintarTablero() {
  // --- Añadir instrucciones y layout ---
  // Si no existe el layout, lo creamos y movemos el board dentro
  let layout = document.querySelector(".layout");
  if (!layout) {
    layout = document.createElement("div");
    layout.className = "layout";

    // Mover el tablero dentro del layout
    if (board.parentElement) {
      board.parentElement.insertBefore(layout, board);
      layout.appendChild(board);
    }

    // Crear y añadir las instrucciones
    const instrucciones = document.createElement("div");
    instrucciones.className = "instrucciones";
    instrucciones.innerHTML = `
      <h1>Instrucciones</h1>
      <ol>
        <li>Selecciona una de las 4 reinas con el borde verde y selecciona una de las casillas con el borde rojo para moverla.</li>
        <li>De la reina que se ha movido, selecciona otra de las casillas con el borde rojo, para bloquear el camino, y que quede inservible.</li>
        <li>Ahora lo mismo con las reinas del otro color.</li>
        <li>El ganador será el que bloquee los movimientos del jugador contrario.</li>
      </ol>
      <button id="newGame" >Juego nuevo</button>
      `;
    layout.appendChild(instrucciones);
  }
  ...
  if (btnNewGame) {
    btnNewGame.onclick = pintarTablero;
  }
}

// Muestra los caminos de la reina selecionada
function mostrarCaminos(celda) {
  const fila = Array.from(board.children).findIndex((row) =>
    Array.from(row.children).includes(celda)
  );
  ...
  if (celda.classList.contains("bordeVerde")) {
    ...
    for (let k = col; k < board.children[fila].children.length - 1; k++) {
      ...
      if (
        celdaActual.children.length > 0 ||
        celdaActual.classList.contains("flecha")
      )
        break;
    ...
    for (
      let k = 1;
      fila + k < board.children.length &&
      col + k < board.children[fila + k].children.length;
      k++
    ) {
    ...
    for (
      let k = 1;
      fila - k >= 0 && col + k < board.children[fila - k].children.length;
      k++
    ) {
      const celdaActual = board.children[fila - k].children[col + k];
      if (
...
document.addEventListener("click", (event) => {
  if (event.target.tagName === "IMG") {
    ...
    event.target.classList.contains("borde") &&
    bloquarCamino == false
  ) {
